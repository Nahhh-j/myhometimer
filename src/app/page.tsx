...
export default function Home() {
  ...
  return (
    <main className="max-w-md mx-auto min-h-screen bg-[#f2f4f6] shadow-lg overflow-hidden relative flex flex-col">
      ...
      {step === 0 && (
        <div className="flex-1 flex flex-col relative animate-fade-in">
          <div className="pt-20 px-8 pb-10 z-10">
            ...
            <h1 className="text-4xl font-extrabold text-gray-900 leading-tight mb-4">내 집 마련,<br /><span className="text-blue-600">몇 년</span> 남았을까?</h1>
            ...
            <div className="relative w-full">
              <div className="flex w-max animate-scroll-left space-x-4 px-4">
                {[...APARTMENT_DATA.slice(0, 20), ...APARTMENT_DATA.slice(0, 20)].map((apt, index) => (
                  <div key={`row1-${index}`} className="flex-shrink-0 w-48 bg-white p-5 rounded-3xl shadow-sm border border-gray-100">
            ...
            <div className="relative w-full">
              <div className="flex w-max animate-scroll-right space-x-4 px-4">
                {[...APARTMENT_DATA.slice(20, 40), ...APARTMENT_DATA.slice(20, 40)].map((apt, index) => (
                  <div key={`row2-${index}`} className="flex-shrink-0 w-48 bg-white p-5 rounded-3xl shadow-sm border border-gray-100">
          ...
          <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-[#f2f4f6] via-[#f2f4f6] to-transparent max-w-md mx-auto z-20">
            <button onClick={goNext} className="w-full py-5 rounded-2xl font-bold text-xl bg-blue-600 text-white hover:bg-blue-700 transition-all shadow-xl">시작하기</button>
          </div>
        </div>
      )}
      ...
      {step > 0 && (
        <>
          ...
          <div className="flex-1 overflow-y-auto pb-32 no-scrollbar">
            {step === 1 && (
              <div className="p-6 animate-fade-in">
                ...
                <div className="mt-8 flex gap-2 mb-6">
                    <div className="flex-1 bg-white p-4 rounded-2xl flex items-center text-gray-400 shadow-sm">
                    ...
                    <button 
                        ...
                        className="bg-white px-4 rounded-2xl shadow-sm border border-transparent hover:border-blue-500 text-blue-600 transition-all flex flex-col items-center justify-center min-w-[70px]"
                    >
                        <Plus size={20} />
                        <span className="text-[10px] font-bold">직접입력</span>
                    </button>
                </div>
                ...
                <div className="space-y-4">
                  {filteredApartments.map((apt) => (
                    <div
                      ...
                      className={`p-5 rounded-3xl border transition-all cursor-pointer flex justify-between items-center bg-white shadow-sm ${
                        selectedApt?.id === apt.id ? 'border-blue-500 ring-4 ring-blue-50' : 'border-transparent'
                      }`}
                    >
                      <div>
                        <div className="text-xs text-gray-400">{apt.region}</div>
            ...
            {step === 2 && (
              <div className="p-6 animate-fade-in">
                <h1 className="text-2xl font-bold text-gray-900 mb-2">자산을 입력해주세요</h1>
                ...
                <div className="space-y-10 bg-white p-8 rounded-[2.5rem] shadow-sm">
                  <div>
                    ...
                    <div className="flex items-center border-b-2 border-gray-100 focus-within:border-blue-500 pb-3 transition-colors">
                      <input type="number" placeholder="0" value={assets.seed} onChange={(e) => setAssets({ ...assets, seed: e.target.value })} className="w-full text-3xl font-bold text-gray-900 outline-none placeholder-gray-200" />
                  ...
                  <div>
                    ...
                    <div className="flex items-center border-b-2 border-gray-100 focus-within:border-blue-500 pb-3 transition-colors">
                      <input type="number" placeholder="0" value={assets.saving} onChange={(e) => setAssets({ ...assets, saving: e.target.value })} className="w-full text-3xl font-bold text-gray-900 outline-none placeholder-gray-200" />
                      <span className="text-xl font-bold text-gray-900 ml-2">원</span>
                    </div>
                  </div>
                </div>
            ...
            {step === 3 && (
              <div className="p-6 animate-fade-in flex flex-col items-center">
                <div className="w-full bg-white rounded-[2.5rem] p-8 shadow-sm text-center mb-6">
                  ...
                  <h1 className="text-4xl font-black text-gray-900 mb-4">앞으로 <span className="text-blue-600">{calculateResult()}년</span></h1>
                ...
                <div className="w-full bg-blue-600 rounded-3xl p-6 text-white shadow-lg">
                  <p className="text-blue-100 text-sm mb-1 font-bold">나희님을 위한 꿀팁 💡</p>
                  <p className="font-bold leading-relaxed">대출을 40% 활용하면 기간을 약 {Math.floor(calculateResult() * 0.4)}년 단축할 수 있어요!</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="fixed bottom-0 left-0 right-0 p-6 bg-white border-t border-gray-50 max-w-md mx-auto z-20">
            <button
              ...
              className={`w-full py-5 rounded-2xl font-bold text-xl flex justify-center items-center transition-all ${
                ((step === 1 && selectedApt) || (step === 2 && assets.seed && assets.saving) || step === 3)
                  ? 'bg-blue-600 text-white shadow-lg active:scale-[0.98]'
          ...
          <div className="w-full max-w-md bg-white rounded-t-[2.5rem] p-8 animate-slide-up shadow-2xl">
            ...
            <div className="space-y-8 mb-10">
              <div>
                ...
                <input type="text" placeholder="예: 우리집" className="w-full border-b-2 border-gray-100 focus:border-blue-500 outline-none pb-2 font-bold text-xl text-gray-900" value={customInput.name} onChange={(e) => setCustomInput({...customInput, name: e.target.value})} />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-400 block mb-2 uppercase tracking-tighter">매매 가격 (원)</label>
                <input type="number" placeholder="예: 500000000" className="w-full border-b-2 border-gray-100 focus:border-blue-500 outline-none pb-2 font-bold text-xl text-gray-900" value={customInput.price} onChange={(e) => setCustomInput({...customInput, price: e.target.value})} />
                <p className="text-sm text-blue-500 mt-2 font-medium">{customInput.price ? formatKoreanCurrency(Number(customInput.price)) : '가격을 입력해주세요'}</p>
              </div>
            </div>
